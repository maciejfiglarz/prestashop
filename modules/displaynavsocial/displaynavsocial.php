...
class DISPLAYNAVSOCIAL extends Module
{
	public function __construct()
	{
		$this->name = 'displaynavsocial';
		$this->description = 'displayNavSocial';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Displaynavsocial_MONOLANGUAGE';
		$this->text_editor_content = 'Displaynavsocial_TEXTEDITOR';
		$this->content_wrapper = 'Displaynavsocial_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Displaynavsocial_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Displaynavsocial_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('displayNavSocial');
		$this->description = $this->l('displayNavSocial');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !DISPLAYNAVSOCIALModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !DISPLAYNAVSOCIALModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = DISPLAYNAVSOCIALModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = DISPLAYNAVSOCIALModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					DISPLAYNAVSOCIALModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					DISPLAYNAVSOCIALModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																DISPLAYNAVSOCIALModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class DISPLAYNAVSOCIALModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'displaynavsocial',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( DISPLAYNAVSOCIALModel::createFilesTable()
				&& DISPLAYNAVSOCIALModel::createContentTable());
	}
